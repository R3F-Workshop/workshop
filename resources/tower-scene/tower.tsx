...
/** The summit beacon: two opposed fake-volumetric beams. */
...
export function Tower({
  ...
  // `useGLTF` suspends, so reaching this effect means the geometry exists and the meshes have committed.
  useEffect(() => {
    onReady?.();
  }, [onReady, nodes]);
  ...
  // One declarative material per mode.
  const modeProps = {
    glow: {
      color: glowBaseColor,
      emissive: "#ff9f3f",
    ...
    sparkle: {
      // Dark iron base: the glitter is entirely the emissive node feeding the pipeline's emissive MRT attachment (→ bloom).
