...
/** A grid of tiles that flip to gold as the cursor sweeps them, and hold that pose for a few seconds before falling back. */
...
/** The largest timestep the spring integrator is allowed to see. */
const MAX_DT = 1 / 20;

/** three's TSL types tag every node with its GLSL type. */
...
export function FlipGrid({
  ...
  // `useThree` types `renderer` as the WebGL/WebGPU union even on the /webgpu entry, and `compute` only exists on the WebGPU one.
  const renderer = useThree((s) => s.renderer) as unknown as WebGPURenderer;

  // One cell spans whichever axis needs more coverage, so the grid always overfills rather than leaving a margin.
  const step = Math.max(viewport.width / cols, viewport.height / rows);

  // Built with TSL's own `uniform()` rather than from raw values.
  const u = useMemo(
    () => ({
      uStep: uniform(1),
      uTile: uniform(1),
    ...
    // Created once for the life of the component: every later change is a value write below, not a rebuild.
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [],
  );

  // Scoped, because uniforms resolve against the *primary* store: on the site this canvas shares one renderer with the hero.
  useUniforms(() => u, "flipGrid");

  // Writing values instead of rebuilding the graph is what makes the Leva panel feel instant rather than recompiling a shader per slider tick.
  useEffect(() => {
    u.uStep.value = step;
  ...
  // Registered at the root with a prefixed key rather than in a "flipGrid" scope, because r3f names scoped storage `scope.name`.
  ...
  const nodes = useLocalNodes(() => {
    ...
    /** Distance to the segment the cursor swept this frame, not to where it happens to be right now. */
    ...
    const update = Fn(() => {
      ...
      // Heavier tiles accelerate more slowly into the flip and overshoot more on arrival, so a sweep breaks up into a ripple instead of a wavefront.
      const mass = float(1).add(hash(instanceIndex).mul(u.uMassJitter));

      angVel.addAssign(
        target.sub(angle).mul(u.uStiffness).div(mass).mul(u.uDt),
      );
      // Exponential decay rather than a bare multiply, so damping means the same thing whatever framerate this canvas ends up running at.
      angVel.mulAssign(exp(u.uDamping.mul(u.uDt).negate()));
      angle.addAssign(angVel.mul(u.uDt));
    })().compute(count);

    // Read-only here: three forces storage access to `read` outside the compute stage, so one node serves both without any juggling.
    ...
    // Which face of the box a fragment belongs to, decided from the *unrotated* normal: the geometry's own identity.
    ...
    // Add roughness variation per facet.
    ...
    /** A gentle dome across each tile: the single thing that makes this read as metal rather than as gold paint. */
    ...
    const perturbed = normalize(
      vec3(
        ...
        // Sign follows the face, so the perturbation leans out of whichever side we're looking at rather than into it.
    ...
    // Roughness varies three ways: per facet, per tile, and by face.
    ...
    return {
      ...
      // The normal has to turn with the tile or the lighting won't sell the flip.
      normalNode: transformNormalToView(spin(faceNormal)),
      colorNode: base,
      metalnessNode: metalness,
      roughnessNode: roughness,
      // Use reflected light without emissive fill.
    };
  });

  const pointer = useRef(new Vector2(AWAY, AWAY));
  /** Set when the cursor teleports: entering the element, or leaving it. */
  ...
  useEffect(() => {
    ...
    /** Moves the cursor body off screen. */
    const park = () => {
    ...
    // Listen on the window rather than the element: the canvas doesn't take pointer events.
  ...
  useFrame((_, delta) => {
    ...
    // On a teleport the segment collapses to a point, so nothing between the old and new cursor positions gets swept.
    ...
    // A specular highlight that travels is one of the strongest metal cues there is: a static one reads as a painted-on shine.
    ...
    // `useFrame` runs in the scheduler's update phase, ahead of this canvas's render, so the vertex stage always reads angles the compute pass just wrote.
    renderer.compute(nodes.update);
  });

  return (
    <>
      {/* Keep resting tiles dark. */}
      ...
      <instancedMesh
        ...
        // Instance transforms live in the shader, so the CPU-side bounding volume is meaningless here.
        frustumCulled={false}
      >
        {/* A unit box, scaled in the vertex stage: so the thickness slider is a uniform write rather than a geometry rebuild. */}
