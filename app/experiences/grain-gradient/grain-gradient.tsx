 ...
 *
 * The whole experience: a canvas, the field inside it, and the dials that drive
 * it. Drop `<GrainGradient />` into any box and it fills it.
 *
 * Modelled on Paper Shaders' grain gradient — written in TSL rather than
 * ported, but following its structure, which gets two things right that are
 * easy to get wrong.
 *
 ...
 * the controls take effect without recompiling the shader on each drag. They
 * are Leva controls, namespaced because Leva's store is global. This file never
 * mounts the panel itself. Whatever hosts the experience decides whether the
...
function GrainField({ params }: { params: GrainParams }) {
...
export function GrainGradient() {
  const params = useControls("grain gradient", {
    ...
    offsetX: { value: 0, min: -1.5, max: 1.5, step: 0.01 },
    offsetY: { value: 0, min: -1.5, max: 1.5, step: 0.01 },
    color1: "#22222a",
    color2: "#6e6e7a",
    color3: "#c8c8d4",
  });
