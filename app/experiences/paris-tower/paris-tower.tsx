...
import {
  CONFERENCE_DAY_OF_YEAR,
  PARIS_LATITUDE,
  TowerCanvas,
} from "./tower-canvas";

/**
 * The finished hero scene with every dial exposed.
 *
 * The whole experience: the tower pipeline (`TowerCanvas`, the MRT post
 * graph, FSR3, the @pmndrs/sky atmosphere) behind the full Leva surface, plus
 * the perf readout. Drop `<ParisTower />` into any box and it fills it.
 *
 * The perf readout is part of what this teaches (the pipeline budget) and it
 * doubles as the liveness and framing diagnostic that debugged the camera
 * more than once. Positioned under a demo page's title, out of the way of
 * the info button (bottom-left) and the controls button (top-right).
 */
export function ParisTower() {
  ...
  return (
    <div className="absolute inset-0">
      <TowerCanvas
        ...
        shadows={shadows}
        // Uncapped: the perf readout below measures pipeline throughput, and a
